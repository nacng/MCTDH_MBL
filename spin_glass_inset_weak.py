...
def q_inset_weak():
    ...
    for n in ['24', '48', '96']:
        ...
        ttt = c1
        logpow = 1.1
        logindx = np.unique(np.floor(np.logspace(1, np.log(len(ttt) - 1)/np.log(logpow) - 1, base=logpow, endpoint=True)).astype('int'))
        vals = c2[logindx]
        m2 = c3[logindx]
        numruns = c1[0]
        stdErr = m2/np.sqrt(numruns)
    ...
    fig.tight_layout()
    fig.savefig('qInset_weak.pdf', transparent=True)
